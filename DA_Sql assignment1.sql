...
-- Answer: In Actor table primary key is "Actor id" and no foreign key. In Actor_award table, Actor_Award_Id is primary key and "Actor id" is foreign key for this table with reference to actor table. In Address table, "Address_id" is primary key & city id is a foreign key but for City table, "City_Id" is primary key
-- Primary key- Primary key is nothing but a unique identifer for every record in database
-- foreign key- is a nothing but a primary key for a table with refernce to another table
