...
-- Question  **Join Practice:**
-- Write a query to display the customer's first name, last name, email, and city they live in.
Select first_name, last_name, email, city from customer inner join 
address on customer.address_id = address.address_id inner join city on city.city_id= address.city_id;
...
-- Include customers who have never rented a film
 Select concat(first_name, " ", last_name) As customer_name, rental_date, title as film_title from 
...
Select title, count(film_actor.film_id) from film 
inner join film_actor on film.film_id = film_actor.film_id group by film_actor.film_id; -- (used joins)
...
select * from customer; -- customername, email, customer_id
Select * from film; -- filmid, rental duration, rate_rate
Select * from rental; -- rental_id, rental_date, return_date inventory_id
select * from inventory;
select concat(first_name, " ", last_name) as customer_name, email, title, rental_date, return_date from customer c 
inner join rental r on c.customer_id = r.customer_id inner join film f on r.last_update = f.last_update;
...
-- Question 7. Show the total number of rentals made by each customer, along with their first and last names.
...
-- question 8 List the customers who have made more rentals than the average number of rentals made by all customers.
SELECT
    first_name,
    last_name,
...
-- Question 9. Display the customer first name, last name, and email along with the names of other customers living in the same city.
SELECT
